...
Pre-built images are available at `alanogic/mcp-odoo-v9`:

```bash
# Pull images
docker pull alanogic/mcp-odoo-v9:latest      # STDIO transport
docker pull alanogic/mcp-odoo-v9:sse         # SSE transport
docker pull alanogic/mcp-odoo-v9:http        # HTTP transport
...
docker build -t alanogic/mcp-odoo-v9:sse -f Dockerfile.sse .
docker build -t alanogic/mcp-odoo-v9:http -f Dockerfile.http .

# Push to Docker Hub
docker push alanogic/mcp-odoo-v9:latest
docker push alanogic/mcp-odoo-v9:sse
docker push alanogic/mcp-odoo-v9:http
```
...
services:
  odoo-mcp-production:
    image: alanogic/mcp-odoo-v9:sse
    environment:
...
services:
  ...
  odoo-mcp-http:
    image: alanogic/mcp-odoo-v9:http
    env_file: .env
    ports:
      - "8008:8008"
    volumes:
